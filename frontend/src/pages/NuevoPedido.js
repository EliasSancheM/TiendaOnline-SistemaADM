...
function NuevoPedido() {
  ...
  const clienteIdFromLocation = location.state?.clienteId;
  
  const [pedido, setPedido] = useState({
    cliente_id: clienteIdFromLocation || '',
    fecha: new Date(),
    periodo: 'mañana',
    estado: 'pendiente',
    notas: '',
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      // Si hay un cliente preseleccionado desde la ubicación
      if (clienteIdFromLocation) {
        const cliente = clientesData.find(c => c.id === clienteIdFromLocation);
        if (cliente) {
  ...
  }, [clienteIdFromLocation, authenticatedFetch]);
  ...
  const vaciarBandeja = () => {
    if (pedido.detalles.length === 0) {
      setSnackbar({
        open: true,
        message: 'La bandeja ya está vacía',
        severity: 'info',
      });
      return;
    }
    
    if (window.confirm('¿Está seguro de que desea vaciar toda la bandeja de productos?')) {
      setPedido({ ...pedido, detalles: [] });
      setSnackbar({
        open: true,
        message: 'Bandeja vaciada exitosamente',
        severity: 'success',
      });
    }
  };
